...
rows = []
for c in train:
    other = [h for h in train if h != c][0]
    r = torch.load(os.path.join(CKPT, f's6_proto2_fold_{other}.pt'),
                   weights_only=False, map_location='cpu')
    Lh, _, _ = s6.space_for(other, 'fold')
...
d = pd.DataFrame(rows)
d.to_csv(os.path.join(HERE, 'ceiling_seen.csv'), index=False)
